...
static int cmd_hello(int argc, char **argv){
    ...
    // 1. 灵魂注入：BnL 启动画面与 WALL-E 状态
    printf("\n");
    printf("  ____        _           _                            \n");
    printf(" |  _ \\      | |         | |                           \n");
    printf(" | |_) |_   _| |__  _ __ | |    __ _ _ __ __ _  ___    \n");
    printf(" |  _ <| | | | '_ \\| '_ \\| |   / _` | '__/ _` |/ _ \\   \n");
    printf(" | |_) | |_| | | | | | | | |__| (_| | | | (_| |  __/   \n");
    printf(" |____/ \\__,_|_| |_|_| |_|_____\\__,_|_|  \\__, |\\___|   \n");
    printf("                                          __/ |        \n");
    printf("     Buy n Large Corporation OS v2.0     |___/         \n");
    printf("\n");
    ...
    printf("Firmware OS  : ESP-IDF %s\n", esp_get_idf_version());
    printf("Core Brain   : ESP32-S3 (Cores: %d, Rev: %d)\n", chip_info.cores, chip_info.revision);
    
    printf("Memory Status: %" PRIu32 " bytes free (Min: %" PRIu32 " bytes)\n", 
            esp_get_free_heap_size(), esp_get_minimum_free_heap_size());
    printf("Flash Drive  : %" PRIu32 " MB %s\n", flash_size / (1024 * 1024),
            (chip_info.features & CHIP_FEATURE_EMB_FLASH) ? "Embedded" : "External");

    printf("Solar / AFE  : [Warning: Solar Array Disconnected - AFE Integration Pending]\n");
    // 结合 3D 打印组装进度
    printf("Chassis Track: [Offline - Mechanical Assembly In Progress]\n");
    printf("Primary Dir. : M-O-C-C (Make Our Code Clean)\n"); // 幽默化 Directive
    printf("==========================\n\n");

    ESP_LOGI("WALL-E", "Boot sequence complete. Ready for input.\n");
    return 0;
}
...
void register_motor_set(void){
    ...
    motor_set_args.duration = arg_int0("t", NULL, "<duration_ms>", "Duration for smooth speed change (ms, optional)");
...
static int cmd_motor_stop(int argc, char **argv){
    ...
    int motor_num = motor_stop_args.motor_num->ival[0];
    ...
    esp_err_t err = DC_Motor_Stop((motor_num == 1) ? MOTOR_RIGHT : MOTOR_LEFT);
    if(err != ESP_OK){
        ESP_LOGE("MOTOR_CMD", "Failed to stop motor (err=%d)!!!", err);
        return 1;
    }
    ESP_LOGI("MOTOR_CMD", "Motor %d stopped successfully.\n", motor_num);
    return 0;
}

void register_motor_stop(void){
    motor_stop_args.motor_num = arg_int1(NULL, NULL, "<motor_num>", "Motor number (0 for left, 1 for right)");
    motor_stop_args.info = arg_lit0(NULL, "info", "Print current motor speeds before stopping");
    motor_stop_args.end = arg_end(3);

    const esp_console_cmd_t cmd = {
        .command = "motor_stop",
        .help = "Stop the specified motor immediately. Usage: motor stop <motor_num> [-v]",
        .hint = NULL,
        .func = &cmd_motor_stop,
